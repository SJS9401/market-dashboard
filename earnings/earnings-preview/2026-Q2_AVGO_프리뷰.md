...
> 작성 시각: 2026-05-25 16:00 KST (v1.0) | **갱신: 2026-06-01 18:30 KST (v1.1 — 셀사이드 2종 반영)**
...
→ **🚨 6/1 Susquehanna 신규 risk: Anthropic $100억 TPU 주문에서 rack 매출 제외 → 칩 공급 ~$25억만 → FY26 AI 매출 컨센 $625억 → $550억 하향 (-12%) + Q3 가이던스 다소 보수적 가능성**
→ **6/1 Oppenheimer는 정반대 view: Q3 가이던스 상향 예상 + AI 매출 Q2 QoQ +30%** ($8.4B → $10.9B) + FY26 AI $570억+ 가능
→ **두 셀사이드 view 갈림 → 시나리오 재조정**: Dell-like Strong/Mega 확률 ⬇ (15% → 5~10%), Normal Beat 확률 ⬆ (35% → 40~45%)
...
### ③ 6/1 신규 셀사이드 리포트 ⭐ (v1.1 추가)

#### (3-1) Oppenheimer — 매수 유지, TP **$450** (현재가 대비 +3.2%)

→ Analyst: **릭 셰이퍼**
→ **"AI가 주도하는 FY26 Q2 실적과 Q3 가이던스 상향 예상"** — Bull view
→ FY26 Q2 AI 매출 **전분기 대비 +30% QoQ** ($8.4B → **$10.92B**) — 가이던스 $10.7B 대비 +2% above
→ AI 가속기 시장 #2 (NVDA 다음), 커스텀 ASIC(XPU) + 네트워킹이 핵심 성장 동력
→ **공급 물량 '28년까지 확보** (TSMC 3nm/2nm)
→ **ASIC 고객 6사 명시**: Alphabet · Meta · **ByteDance (틱톡 US 사업부)** · Anthropic · OpenAI · (unnamed)
→ **Anthropic capacity ramp 정량화**: 2026년 **1GW ($150~200억 규모)** → 2027년 **3GW**로 3배 확대
→ Meta MTIA 로드맵 순조롭게 진행 중
→ **OpenAI FY27 (26년 11월~27년 10월) 1GW 연산 용량 배치 예상**
→ **추가 주문 반영 시 FY26 AI 매출 $570억 상회 가능**

#### (3-2) Susquehanna — 매수 유지, TP **$450 → $490 (+9% raise)** (현재가 대비 +12.4%)

→ **"TPU 수요 강세 + 커스텀 XPU 모멘텀 지속 + AI 네트워킹 수요 확대"** 평가
→ **🚨 신규 risk 발견: Anthropic용 초기 TPU 공급 물량에서 rack 제외**
→ "CY26 커스텀 XPU 매출 전망 낮아졌지만, **CY27 전망은 변함없음**" — multi-year secular 유지
→ "**캘린더 기준 1분기 (= AVGO FY26 Q2) 실적은 예상치 상회 전망**" — Q2 beat 확인
→ "FY26 하반기 예정 Anthropic **$100억 규모 TPU 주문에서 rack 매출 제외** 예상 → **캘린더 Q2 (= AVGO FY26 Q3) 가이던스 다소 보수적 제시 가능성**" — Q3 guidance bearish
→ "해당 주문 관련 매출은 칩 공급에 국한 → **약 $25억** 수준" (vs $100억 expected = **-$75억 lost**)
→ **FY26 AI 매출 전망: $625억 → $550억으로 하향** (-12%)
→ **흥미로운 모순**: 매출 하향에도 TP 상향 = "단기 매출 노이즈 < CY27 secular 빌딩"

#### (3-3) 두 셀사이드 view 갈림 — 어느 쪽이 맞나?

| 항목 | Oppenheimer | Susquehanna |
|---|---|---|
| Q2 매출/AI | Beat 예상 | Beat 예상 |
| Q3 가이던스 | **상향 예상** ⬆️ | **보수적 제시 가능성** ⬇️ |
| FY26 AI 매출 | $570억+ 가능 | $550억으로 하향 |
| TP | $450 유지 | $450 → $490 raise |
| 핵심 risk | (별도 언급 X) | **Anthropic rack 제외** |

→ **Q2 beat은 양사 합의**. **Q3 가이던스 방향이 결정적 catalyst** — Anthropic rack 제외 이슈가 실제로 영향 미치면 Susquehanna 시나리오, Hock Tan이 다른 customer order로 만회하면 Oppenheimer 시나리오

### ④ 핵심 narrative — Hock Tan AI $100B by FY27 (v1.1 보강)

(1) **6 named custom-silicon customer 정확 구성**: Google · Anthropic · Meta · OpenAI (Q1 FY26 신규) · **ByteDance (틱톡 US)** · (unnamed)
(2) **Backlog $73B** (Q1 FY26 종료 시점)
(3) **TSMC 3nm + 2nm capacity 확보 — '28년까지** (Oppenheimer 6/1 확인)
(4) **FY27 AI semi $100B 시사** — Q2 가이던스 $10.7B 기준으로 분기 평균 $25B 도달 필요
(5) **🆕 customer별 capacity timing**:
   - Anthropic: 2026년 1GW ($150~200억) → 2027년 3GW
   - OpenAI: FY27 (26.11~27.10) 1GW 배치
   - Meta: MTIA 정상 진행
   - Google: TPU v6 ramp + 비-구글 LLM (Anthropic 등) TPU 활용 확대
...
### ② AVGO Dell-like 시나리오 매트릭스 (v1.1 — 셀사이드 반영 확률 재조정)

![AVGO 발표 후 시나리오](2026-Q2_AVGO_dell_scenario.png)

| 시나리오 | v1.0 확률 | **v1.1 확률** | 매출 vs 가이던스 | AI Semi | Q3 가이던스 | 주가 변동 |
|---|---|---|---|---|---|---|
| Bull (Dell-like Mega) | 10~15% | **5~10% ⬇️** | +12% ($24.6B) | $13B+ | $27B + FY27 $130B reiterate | +25~40% |
| Strong Beat | 30~35% | **20~25% ⬇️** | +7% ($23.5B) | $11.5~12B | $25~26B raise | +15~25% |
| **Normal Beat** | 35~40% | **40~45% ⬆️ ★** | +3% ($22.7B) | $11B | $24B (cons 적중) | +5~10% |
| Conservative | ~15% | **15~20% ⬆️** | +1% ($22.2B) | $10.7B | $22.5B | +2~5% |
| Miss | ~10% | ~10% | -2% ($21.5B) | $10B | guidance 후퇴 | -5~15% |

**v1.1 확률 재조정 사유:**
→ Susquehanna가 발견한 **Anthropic rack 제외 risk** = Q3 step-up 폭 약화 시그널
→ Susquehanna의 FY26 AI 매출 하향 ($625억 → $550억)이 시장에 알려져 Q3 가이던스 expectation 자체가 낮아짐
→ 단, Oppenheimer는 정반대 view → 양면 view 갈림 = 확실성 하락 → 중간(Normal) 확률 증가
...
## 항목 6. 핵심 관전 포인트 (v1.1 — 우선순위 재조정)

### ① 6/3 발표 핵심 catalysts — 6/1 셀사이드 반영 우선순위

(1) **🔴 1순위 — Anthropic rack 매출 인식 여부 (Susquehanna 핵심 risk)**
→ Susquehanna 발견: $100억 주문 중 rack 제외 → 칩만 $25억 인식
→ **Hock Tan이 "Anthropic shipment includes rack" 명시 시 즉시 Strong Beat 복귀** (+15%+)
→ Hock Tan이 "chip-only" 인정 시 Susquehanna 시나리오 confirm → Q3 가이던스 보수적 → +5~10% Normal

(2) **🟡 2순위 — Q3 FY26 매출 가이던스**
→ Cons mid $24B
→ **Susquehanna 시나리오**: $22~23.5B (보수적 — Anthropic rack 제외 반영)
→ **Oppenheimer 시나리오**: $25B+ (상향 — 추가 customer order 만회)
→ **$25B 이상 announce → Susquehanna wrong → +15~25% 상승**
→ **$23B 이하 → Susquehanna right → 일시 매도 -5% 가능**

(3) **🟢 3순위 — AI Semi Q3 가이던스**
→ Q2 $10.7B → Q3 $12B+ 시 secular acceleration 확인
→ $13B+ 시 mega trigger
→ Anthropic rack 빠지면 Q3 AI 가이던스 $11~11.5B 수준 가능 (보수적)

(4) **🟢 4순위 — Anthropic 1GW 매출 인식 분포**
→ FY26 Q2·Q3·Q4 어디에 들어가나? Hock Tan ranging 코멘트
→ 2027년 3GW 확대 시점 reiterate 여부

(5) **🟢 5순위 — OpenAI FY27 1GW 배치 시점 명시 (Oppenheimer 정량화)**
→ "starting in fiscal year 2027" 정확 reiterate → narrative 강화
→ FY27 Q1·Q2 어느 분기부터 매출 인식 시작?

(6) **🟢 6순위 — FY26 annual AI 매출 가이던스 (만약 제시)**
→ $570억+ → Oppenheimer 시나리오 confirm (Strong)
→ $550억 → Susquehanna 시나리오 confirm (Normal)
→ $500억 이하 → Bearish (-10%+ 가능)

(7) **🟢 7순위 — ByteDance (틱톡 US) 매출 인식 + customer 확장**
→ 6번째 named customer로 ByteDance 명시 시 narrative 추가
→ 7번째 customer announce 시 narrative re-rating

### ② 리스크 요인 (v1.1 추가)

(1) **🚨 Anthropic rack 제외 risk** (Susquehanna 6/1) — Q3 AI 매출 expectation -$75억
(2) **Q3 가이던스 cons 적중·아래** → "선반영" sell-on-news 가능 (-5~10%)
(3) **AI Semi Q2 실제가 $10.7B 정확 적중** (+0% above) → 보수성 강화 시그널
(4) **Wireless 약세 가속** (Apple iPhone cycle) → Non-AI semi drag
(5) **Valuation 부담** — Forward PER 30+x, 시총 $2T → 멜팅 inertia
(6) **신고가 직후 차익 실현** — 1년 +80% 누적 후 일부 매도 압력

### ③ 최종 시사 (v1.1)

| 시나리오 | v1.0 | **v1.1** | 핵심 변수 |
|---|---|---|---|
| Dell-like +30%+ | 10~15% | **5~10% ⬇️** | Q3 가이던스 $26B+ + Hock Tan rack inclusive |
| Strong Beat +15~25% | 30~35% | **20~25% ⬇️** | Q3 $25B+ + AI $12B+ |
| **Normal Beat +5~10%** | 35~40% | **40~45% ★** | Q2 beat + Q3 cons 적중 |
| Conservative +2~5% | ~15% | **15~20% ⬆️** | Susquehanna 시나리오 confirm |
| Miss -5~15% | ~10% | ~10% | Wireless 가속 약세 |

**v1.1 핵심 변수 재정의**: 
→ **(a) Anthropic rack 포함/제외 여부** = 1순위 결정타 (어제는 부재)
→ (b) Q3 가이던스 step-up 폭 ($25B 이상 vs 이하)
→ (c) FY27 AI capacity 명시 ($100B beyond 시그널)

**핵심 시그널 — 6/3 Hock Tan 컨퍼런스콜 듣기 포인트:**
1. "Anthropic shipments include rack" 또는 "chip-only" — Susquehanna risk 확인
2. Q3 매출 가이던스 number — $25B 분기점
3. FY27 AI $100B+ 또는 specific number reiterate
4. 7번째 customer 또는 expansion announce

**6/1 두 셀사이드 종합 시사**: Q2 beat 확정 + Q3 방향 갈림. **base case = Normal Beat (+5~10%)**. Dell-like 가능성은 어제 분석보다 ⬇️.
...
→ **v1.1** (2026-06-01 18:30 KST): 6/1 신규 셀사이드 2종 반영. (a) **Oppenheimer 매수 $450 유지** + Q3 가이던스 상향 예상 + AI Q2 QoQ +30% + Anthropic 1GW→3GW capacity 정량화 + OpenAI FY27 1GW 명시 + ByteDance 6번째 customer. (b) **Susquehanna 매수 $450 → $490 raise** + 🚨 Anthropic $100억 주문 rack 제외 risk + Q3 가이던스 보수적 가능성 + FY26 AI 매출 $625억 → $550억 하향. **시나리오 확률 재조정**: Dell-like Strong/Mega 15% → 5~10% ⬇️, Normal Beat 35% → 40~45% ⬆️. 핵심 catalyst 우선순위 재정의 = Anthropic rack 포함/제외 여부가 1순위.
...
→ **Oppenheimer 6/1 리포트 (서치엠글로벌 권기현 기자 인용)**: 매수 $450 유지, 릭 셰이퍼 analyst, AI Q2 +30% QoQ, FY26 AI $570억+ 가능
→ **Susquehanna 6/1 리포트 (서치엠글로벌 권기현 기자 인용)**: 매수 $450 → $490, Anthropic rack 제외 risk, FY26 AI $625억 → $550억 하향
