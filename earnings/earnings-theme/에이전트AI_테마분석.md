...
sunset_companies: []
last_updated: 2026-09-09 (v5.3 — 2016-18 패턴 반복 시나리오 등재 + 공급 측 트래킹 지표)
...
narrative_shift_log:
  ...
  - {date: 2026-Q3, event: "★ AMAT: DRAM 산업 capa 월 160만장 → 200만장 (+25%), 향후 수년간 연 30~40만장 추가 (Citi TMT 2026-09-08). 삼성 P4·SK M15X 본격 가동 + Micron 아시아 D램 전환 결과"}
  - {date: 2026-Q3, event: "★ 삼성증권 사이클 오버레이 — 2016-18 슈퍼사이클 4국면 패턴에 현 사이클 매핑. 현재 2국면 후반, 2027.01 공급확대 진입 = 주가 정점, 2028.01 수요감소. '1년 뒤 치킨게임' 전망 형성"}
  - {date: 2026-Q3, event: "★ v5.3 (2026-09-09): Step 7에 '2016-18 패턴 반복' 리스크 등재 (확률 35%) + 반론 4가지 병기 (핵심: capa +25% ≠ bit +25%, HBM wafer 3배 소비) + Step 8 공급 측 지표 신설 (HBM 흡수율·bit 기준 공급 증가율)"}
---
...
# ★ v5.3 add-on (2026-09-09) — 2016-18 패턴 반복 시나리오 등재

> **trigger**: 삼성증권 사이클 오버레이 차트 (Quantwise / Samsung Securities estimates) + **AMAT Citi TMT 컨퍼런스 발언** (2026-09-08). 시장에서 "1년 뒤 치킨게임" 전망이 형성 중.
>
> **등재 목적**: 본 테마의 **최대 하방 시나리오를 정식 리스크로 편입**하고, 판정 기준을 v5.2 검증 포인트와 연결한다.

---

## A. Step 7 리스크 팩터 — ★ 신규 등재: 2016-18 패턴 반복

### A-1. 시나리오 개요

| 항목 | 내용 |
|---|---|
| **리스크명** | **2016-18 슈퍼사이클 패턴 반복 (공급 확대 → 수요 감소 → 치킨게임)** |
| **확률** | **35%** (기존 "AI capex 변곡점 25%"와 일부 중첩, 공급 측 요인 추가로 상향) |
| **임팩트** | **매우 큼** — 메모리 3사 동시 마진 붕괴, 2028 컨센 하향 |
| **출처** | 삼성증권 사이클 오버레이 차트 + AMAT capa 데이터 (2026-09-08) |

### A-2. 4국면 매핑 (과거 → 현재)

| 국면 | 과거 | **현재 매핑** | 주요 이벤트 |
|---|---|---|---|
| **1. DRAM 가격 상승 모멘텀** | ~2017.01 | ~2026.07 | 가격 반등 → **상승률 정점** |
| **2. 영업이익 지속성** | 2017.01~07 | **2026.07~2027.01 (현재)** | ★ 차트 원문 "**a phenomenon unique to this cycle**" 명시. 삼성 P1 완공 |
| **3. 공급 확대** | 2017.07~ | **2027.01~** | Micron ID1 / SK Y1 / CXMT F1 / 삼성 P5-1 개소 → **주가 정점 후 하락 전환** |
| **4. 수요 감소** | 2018.01~ | **2028.01~** | Amazon 발주 축소, DRAM 가격 하락. CXMT A3·C3 / Micron ID2 / SK Y2 / 삼성 P5-2 |

★ **차트 논리대로면 2027 초 3국면 진입 = 주가 정점**, 2028 초 4국면 = 급락.

### A-3. 정합성 — v5 통합본과 독립적으로 같은 시점을 지목

| v5/v5.2 도출 | 본 시나리오 |
|---|---|
| 3사 2027 동시 증설 (SK 용인 2027-02 / 삼성 P5 2027-05 / Micron 2H27) | 3국면 = 2027.01~ 공급 확대 |
| 4사 CapEx peak **2027 H2 ~ 2028 H1** | 주가 정점 2027 상반기 |
| **2028 수급 반전 risk** | 4국면 = 2028.01~ 수요 감소 |

→ **서로 다른 방법론(빅테크 CapEx trajectory vs 과거 사이클 오버레이)이 같은 창을 가리킴** = 시나리오 신뢰도 상승 요인.

### A-4. ★ 반론 4가지 (등재 시 함께 기록)

**① capa +25% ≠ bit 공급 +25% — 가장 중요한 반론**

- **HBM은 wafer 1장당 bit를 일반 DRAM의 약 1/3만 산출** → 증설 wafer가 HBM으로 흡수될수록 실효 bit 공급 증가는 크게 축소
- v5.2 변수와 중첩: **8-Hi 전환** = die 사용량 1/3 감소 (같은 HBM 물량을 더 적은 wafer로) ↔ **CPX 부활** = HBM 소켓 2배 (wafer 흡수 증가)
- ★ **wafer capa 지표만으로 공급 과잉을 판단하면 과대평가된다.** 본 차트는 이 구분을 하지 않음

**② 차트 스스로 인정하는 차이** — 2국면에 "a phenomenon unique to this cycle" 명시. 작성자도 완전한 반복은 아니라고 봄

**③ 2016-18과의 구조적 차이**

| 차원 | 2016-18 | 현 사이클 |
|---|---|---|
| 수요 성격 | 모바일·서버 일반 수요 | **AI 인프라 (secular)** |
| 물량 가시성 | 없음 | ★ **LTA** (SK 5년 $750B, 삼성-Broadcom $200B) |
| 제품 구조 | DRAM 단일 | **HBM / 일반 DRAM 분화** (가격·마진 체계 별개) |
| 고객 구조 | 분산 | 하이퍼스케일러 소수 + 장기 계약 |

→ 특히 **LTA는 2016-18에 없던 하방 경직 장치**. Amazon 발주 축소가 즉시 가격 붕괴로 이어졌던 당시와 구조가 다름

**④ CXMT 변수는 오히려 지금이 더 큼** — 중국 capa는 가격 논리로 움직이지 않을 수 있어, 이 부분은 차트의 경고가 **과소평가**일 가능성

### A-5. 시나리오가 반영하지 못한 v5.2 변수

| 변수 | 방향 | 비고 |
|---|---|---|
| **CPX 부활** (HBM 소켓 2배, 시스템 360GB) | 수요 ↑ | 차트 작성 이후 등장 |
| **Astra 1M 컨텍스트** (prefill 용량 수요) | 수요 ↑ | 차트 작성 이후 등장 |
| LTA 물량 가시성 | 하방 경직 ↑ | 미반영 |
| capa vs bit 구분 | 공급 과잉 과대평가 | 미반영 |

★ **본 차트는 v5.2에서 정의한 "Before(2028 컨센 의구심)"를 시각화한 것이며, "After(Astra·CPX 트리거)"는 미반영 상태.**

### A-6. ★ 판정 기준 — v5.2 검증 포인트와 연결

본 시나리오의 실현 여부는 **v5.2 F항 두 검증 포인트가 판정한다**:

| 조건 | 결과 |
|---|---|
| ① Astra 수요 base 확장 **성공** + ② CPX 2027 Q1 **실현** | **3국면 공급 확대를 수요가 흡수** → 시나리오 회피 |
| ①② 중 하나만 성립 | 조정은 오되 4국면(급락)까지는 미도달 |
| ①② **모두 실패** | ★ **본 시나리오대로 진행** (2027 정점 → 2028 치킨게임) |

---

## B. Step 8 트래킹 지표 추가 — 공급 측 (v5.3)

> 기존 v5.2 지표(수요 검증·아키텍처 검증·credit)에 **공급 측 실물 지표**를 추가한다.

### B-1. ★ DRAM wafer capa (AMAT 기준)

**Fact (AMAT, 2026-09-08 Citi TMT 컨퍼런스)**:

| 항목 | 수치 |
|---|---|
| 기존 DRAM 산업 capa | 월 웨이퍼 투입 **160만장** |
| 2026년 확대 후 | **200만장** |
| 증가분 | **+40만장 = +25%** |
| 향후 전망 | 수년간 **연 30~40만장** 추가 |
| 원인 | 삼성 P4 본격 가동 · SK 청주 M15X 본격 가동 · Micron 아시아 기존 라인 D램 전환 |

**트래킹 항목**:
- **월 wafer 투입량 추이** (AMAT·장비사 컨퍼런스, 분기별) — 200만장 → 230~240만장 페이스 유지 여부
- ★ **HBM 흡수율** — 증설 wafer 중 HBM 할당 비중. **이 비중이 높을수록 일반 DRAM bit 공급 증가는 축소** (반론 ①의 핵심 검증)
- ★ **bit 기준 공급 증가율** — wafer capa 증가율과의 괴리 폭. TrendForce·3사 컨콜 bit growth 가이던스
- 3사 신규 팹 가동 시점 지연·앞당김 (SK 용인 2027-02 / 삼성 P5 2027-05 / Micron ID1 2H27)
- **CXMT capa 진행** (F1 → A3·C3) — 중국 변수, 가격 논리 밖 공급

### B-2. 사이클 국면 전환 시그널

- **DRAM 가격 상승률 (전월비)** — 2국면 → 3국면 전환은 가격 상승률이 아니라 **절대가격이 꺾이는 시점**
- **3사 재고 수준** (DIO) — 공급 확대의 첫 징후
- **하이퍼스케일러 발주 조정** — 2016-18 4국면 trigger가 Amazon 발주 축소였음. 현 사이클 대응 지표는 빅테크 4사 CapEx 가이던스 (v5 macro layer와 연결)
- **LTA 이행률** — 계약 물량이 실제 인식되는지. 하방 경직 장치의 실효성 검증

---

## v5.3 changelog

**v5.3 (2026-09-09)**: 2016-18 패턴 반복 시나리오 등재
- ★ Step 7 리스크 신규 등재: **2016-18 슈퍼사이클 패턴 반복** (확률 35%, 임팩트 매우 큼)
- 4국면 매핑 (현재 = 2국면 후반, 2027.01 3국면 진입, 2028.01 4국면)
- ★ 반론 4가지 병기 — 특히 **capa +25% ≠ bit +25%** (HBM wafer 3배 소비)
- v5 통합본 peak 추정(2027 H2~2028 H1)과 독립적 정합성 확인
- ★ 판정 기준을 v5.2 검증 포인트(Astra·CPX)와 연결
- ★ Step 8 공급 측 지표 신규: AMAT wafer capa (160→200만장, +25%), **HBM 흡수율**, bit 기준 공급 증가율, CXMT 진행, 사이클 국면 전환 시그널 4건
- 출처: 삼성증권 사이클 오버레이 차트 (Quantwise) + AMAT Citi TMT (2026-09-08)
