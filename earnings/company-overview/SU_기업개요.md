...
> 작성 시각: 2026-05-25 KST (**v1.5** — v1.0~v1.4 + 핸드오프 표준 5가지 retrofit: 12년 손익 표 narrative annotation 추가 (코로나·CMD 2030 marker), 밸류에이션에 삼성전자 비교 1줄 추가, 재평가 트리거를 실적 추적 변수에서 '분류 변경 조건' 3종으로 재정의)
...
![Schneider 12년 매출 & Adj EBITA Margin](charts/SU_chart1_revenue_margin_12y.png)

![Schneider 12-Year P&L 4 Metrics](charts/SU_chart1b_pl_12y.png)

![Schneider Quarterly Revenue & Organic Growth (Q1 24-Q1 26)](charts/SU_chart10_quarterly_9Q.png)

![Schneider Quarterly Revenue by Business](charts/SU_chart2_segment_quarterly.png)

![Schneider Balance Sheet Evolution](charts/SU_chart4_balance_sheet_5y.png)

![Schneider Shareholder Structure](charts/SU_chart5_shareholders.png)

![Schneider Free Cash Flow Trend](charts/SU_chart6_FCF_12y.png)

![Schneider R&D Investment](charts/SU_chart7_RD_12y.png)

![Schneider CapEx Trend](charts/SU_chart8_CapEx_12y.png)

![Schneider Progressive Dividend](charts/SU_chart9_dividend_12y.png)

![Schneider 12-Year P&L Trend](charts/SU_chart12_income_12y.png)
...
```
[Schneider Electric 12년 손익 시계열 (EUR €M, 주요 연도)]
연도    매출       Adj EBITA  Margin   비고
2014   24,939     3,581     14.4%    ← 정상 사이클 phase (Schneider 통합 운영 본격)
2015   26,640     3,902     14.6%
2016   24,693     3,450     14.0%    ← 신흥국 사이클 저점 (12년 Margin 최저)
2017   24,743     3,640     14.7%
2018   25,720     3,895     15.1%
2019   27,158     4,193     15.4%    ← 디지털 전환 본격 (EcoStruxure 가속)
2020   25,159     3,778     15.0%    ← 코로나 영향, 일시 둔화
2021   28,905     4,815     16.7%    ← AVEVA 통합 phase + 코로나 회복
2022   34,176     5,724     16.7%
2023   35,902     6,407     17.8%    ← AVEVA 100% 인수 완료 (industrial software 강화)
2024   38,150     7,100     18.6%    ← Olivier Blum 신임 CEO + Motivair 인수 (AI 데이터센터 cooling)
2025E  42,500     8,100     19.4%    ← CMD 2030 strategy 발표 (Margin +250bp 누적 ambition)

Adj EBITA Margin range: 14.0% ~ 19.4% = +5.4%pt (12년 단조 우상향, downcycle 0회)
사이클 cutoff (±10%pt): 미달 → 지속성장 분류 (4 end-market secular pure-play)
```
...
→ **1차 — Forward PER** (지속성장 글로벌 #1 pure-play): SU 28-30x (premium quality). 글로벌 피어 (ABB 30x · Eaton 32x · GEV 50x) reference, Schneider는 quality·sustainability·software premium 정당화

→ **2차 — EV/EBITDA + DCF (3-stage)**: 19% Adj EBITA Margin + 4-7% revenue growth + FCF €4B+ + 32년 연속 progressive dividend → forward EV/EBITDA 18-20x = €145-160B EV
...
→ **삼성전자 대비**: 삼성전자는 PBR 단독 사이클, SU는 PER 단독 secular 글로벌 #1 (downcycle 부재) — Sustainability·Software 프리미엄 추가 정당화

(7) 분기 재평가 트리거 (**분류 변경 조건** — 실적 추적 변수는 6번 섹션)

→ ① **Industrial Automation segment Adj EBITA Margin range 4 분기 연속 +10%pt 진폭 발생 시** → 지속성장 → **사이클 비중 증가** (IA 18% 비중, Discrete capex 사이클 노출 확대)

→ ② **China 매출 비중 25%+ 상승 시** (현재 18%) → 지정학 사이클 노출 → **분류 재검토** (US-China tension 시 reported Margin 변동성 확대)

→ ③ **EM + IA Lumada·AVEVA·Motivair·EcoStruxure 통합 비중 70%+ 도달 시 (Digital Flywheel CMD 2030 target) + Software & Services 25%+ 도달** → 지속성장 → **Pure SaaS secular 격상** (PER 35x+ premium 적용 가능)
...
![Schneider Electric 12년 매출 & Adj EBITA Margin](charts/SU_chart1_revenue_margin_12y.png)

(3) 주가 역사 (20년)

![Schneider Electric Market Cap (Euronext SU.PA)](charts/SU_chart11_market_cap_20y.png)
...
![Schneider Electric Product Matrix](charts/SU_chart3_product_matrix.png)
...
- **v1.5 (2026-05-25)** — 핸드오프 표준 5가지 retrofit. 12년 손익 표 narrative annotation 추가 (EcoStruxure·코로나·AVEVA 통합·Blum CEO·CMD 2030 변곡점 marker), 밸류에이션에 삼성전자 비교 1줄 추가 ("PER 단독 secular 글로벌 #1, downcycle 부재"), 재평가 트리거를 실적 추적 변수에서 '분류 변경 조건' 3종으로 재정의 (IA 사이클 비중 증가 / China 25%+ 지정학 재검토 / Software & Services 25%+ Pure SaaS 격상)
