...
→ **1차 — EV/EBITDA + DCF** (턴어라운드 분류 기반, FCF gen 가속): GEV Adj EBITDA 가속 (FY25 $3.06B → FY28 $7-8B 가이던스), EV/EBITDA 25-30x — 턴어라운드 trajectory reflection
...
→ **삼성전자 대비**: 삼성전자는 PBR 사이클, GEV는 EV/EBITDA + DCF 턴어라운드 (분사 직후 P/B 의미 없음) — Wind 흑전 시 PER 단독 평가로 transition 예상

(7) 분기 재평가 트리거 (**분류 변경 조건** — 실적 추적 변수는 6번 섹션)

→ ① **Wind segment 2 분기 연속 흑자 + Adj EBITDA Margin 5%+ 안정 시** → 턴어라운드 → **지속성장 격상** (Wind drag 해소 = 핵심 분류 변경 trigger)

→ ② **Power+Wind+Electrification 통합 4 분기 연속 매출 +10% YoY + Adj EBITDA Margin 14%+ 도달 시** (FY28 CMD 가이던스 trajectory) → **Primary 지속성장 + Secondary 사이클 재분류** (ABB·Schneider 동등 multiple 적용)

→ ③ **Wind segment 다시 적자 확대 (-30% 이상) + Power Margin 감소 + Gas backlog 110GW 미달 시** → 턴어라운드 retrace → **Cyclical 강등** (Capex cycle 의존성 재확인)
...
- **v1.5 (2026-05-25)** — 핸드오프 표준 5가지 retrofit. 4년 손익 표 narrative annotation 추가 (GE 분사·Form 10·standalone 첫 해·Wind 적자 정상화 marker), 밸류에이션에 삼성전자 비교 1줄 추가 ("EV/EBITDA + DCF 턴어라운드, 분사 직후 P/B 의미 없음"), 재평가 트리거를 실적 추적 변수에서 '분류 변경 조건' 3종으로 재정의 (Wind 흑전 지속성장 격상 / FY28 CMD 14% Margin Primary 지속성장 / Wind 적자 확대 Cyclical 강등)
