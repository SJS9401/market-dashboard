...
> 섹터: 전력 인프라 (T1 메인)
> 작성 시각: 2026-05-25 KST (**v1.5** — v1.0~v1.4 + 핸드오프 표준 5가지 retrofit: 12년 손익 표 narrative annotation 추가 (사이클 저점·재침체·secular 진입 marker), 밸류에이션에 삼성전자 비교 1줄 추가, 재평가 트리거를 실적 추적 변수에서 '분류 변경 조건' 3종으로 재정의)
...
```
[LS ELECTRIC 12년 별도 손익 시계열 (억원)]
연도    매출       OP       OPM     NI
2014   18,277    1,551    8.5%    939         ← 정상 사이클 phase (LS산전 시절, 변압기·차단기 위주)
2015   17,531    1,479    8.4%    695
2016   17,124      974    5.7%    579         ← 12년 사이클 저점 (LS산전 마지막 phase)
2017   18,075    1,431    7.9%    948
2018   19,051    1,748    9.2%   1,194        ← 1차 회복 정점 (사이클 1회차 완료)
2019   18,101    1,547    8.5%   1,094
2020   18,546    1,132    6.1%   1,062        ← 코로나 + ESS 사업 적자 → 재침체 phase 진입
2021   18,774    1,147    6.1%    710         ← 사명 변경 (LS산전 → LS ELECTRIC, 2020)
2022   22,835    1,382    6.1%    674         ← 3년 연속 OPM 6.1% 박스권 (사이클 2차 저점)
2023   30,043    2,784    9.3%   2,025        ← Secular 진입 (미국 배전 사이클 + 빅테크 데이터센터 수주 시작)
2024   31,085    3,127   10.1%   2,134        ← 가속 (DC 솔루션·자동화 secular 본격)
2025   34,627    3,622   10.5%   2,938        ← 빅테크 X사 3,530억 + 부산공장 증설 + LS파워솔루션 인수

OPM range: 5.7% ~ 10.5% = +4.8%pt (cutoff ±10%pt 미달 → 약한 사이클)
사이클 회수: 1회 (2016 저점 → 2018 9.2% → 2020-22 6.1% 재침체 → 2023~ secular)
3년 OPM 단조 우상향 (FY22-25 +4.4pp) + DC·자동화 secular 진입 = boundary case
```
...
→ **1차 — Forward PER + EV/EBITDA** (사이클 + secular boundary 분류 기반): 12MF EPS 기준 글로벌 피어 PER reference, 한국 3사 중 multiple 가장 빠르게 리레이팅 진행 (transition phase)

→ **2차 — P/B 밴드** (사이클 잔여): 별도 자본 12년 CAGR 7% trajectory + ROE 14.8% → P/B band 추적 (사이클 분류 잔존 시 활용)

→ **3차 — 사이클 매핑** (Forward OPM trajectory): FY25 10.5% (별도) → FY28 14% 시 글로벌 피어 격차 축소 + multiple expansion

→ **DCF/SOTP 미사용 근거** — 자회사 다수이나 OP 기여 mix 작음 (LS 본체 84% OP 기여) → SOTP 분해 효익 작음

→ **삼성전자 대비**: 삼성전자는 PBR 사이클, LS일렉트릭은 PER + EV/EBITDA (DC·자동화 secular 진입 중 transition phase) — 사이클 → 지속성장 boundary에서 PER 적용 비중 점진 확대

(7) 분기 재평가 트리거 (**분류 변경 조건** — 실적 추적 변수는 6번 섹션)

→ ① **DC(직류)·자동화 매출 비중 40%+ 도달 + 4 분기 연속 OPM 12%+ 안정 시** → 사이클 → **지속성장 격상** (LVDC·MVDC·HVDC SST·SSCB + 자동화 secular trajectory 확정, 한국 후발 → 글로벌 피어 동등)

→ ② **전력기기 사이클 진입 (OPM range +10%pt 이상 진폭 발생)** 시 → **사이클 분류 강화** (현재 약한 사이클 +4.8pp → 강한 사이클로 재정의, 2020-22 6.1% 박스권 재진입 risk)

→ ③ **글로벌 진출 매출 비중 30%+ 도달 + 빅테크 패키지 수주 누적 3조 도달 시** → **한국 후발 → 글로벌 피어 분류 격상** (효성·HD 동등 글로벌 점유율 trajectory, 자동화·DC secular pure-play 위상 확보)
...
- **v1.5 (2026-05-25)** — 핸드오프 표준 5가지 retrofit. 12년 손익 표 narrative annotation 추가 (LS산전 사이클 저점·LS ELECTRIC 사명 변경·6.1% 박스권·secular 진입·빅테크 수주 marker), 밸류에이션에 삼성전자 비교 1줄 추가 ("PER + EV/EBITDA, DC·자동화 secular 진입 중 transition phase"), 재평가 트리거를 실적 추적 변수에서 '분류 변경 조건' 3종으로 재정의 (DC·자동화 40%+ 지속성장 격상 / OPM range +10%pt 사이클 강화 / 글로벌 30%+ 한국 후발 → 글로벌 피어 격상)
...
- **v1.3 (2026-05-18 16:30)**: **DART 본문 사업보고서 자동 fetch 완료** — 한국 기업 개요 작성 시 source 6종 전수 점검 룰 첫 적용
  ...
  - **임원 전체 명단 신규**: 사내이사 4명 + 사외이사 5명 + 미등기임원 30+명 (등기여부·임기·학력·