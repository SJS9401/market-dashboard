#!/usr/bin/env python3
"""KRX 일별 K200+Q150 OHLC 백필 (2026-08-23, 눌림 전략 검증용).

kr_k200q150_backfill.py 와 동일 유니버스 (그날 시총 상위 200+150, 생존편향 없음).
저장: [open, high, low, close(원, 미조정), rt(%, 분할 조정 반영)] — 분할은 rt 로 조정 계열 재구성.
출력: data/kr_k200q150_ohlc.json.gz
  { updated, last_date, codes: {code: [name, mkt]}, dates: [...],
    days: [ {code: [o,h,l,c,rt]}, ... ] }
...
OUT = SCRIPT_DIR / "data" / "kr_k200q150_ohlc.json.gz"
...
START = "20140101"
MAX_DAYS = int(os.environ.get("MAX_DAYS", "3300"))
TOPN = {0: 200, 1: 150}
            ...
            if mk and total > 1200:
                ...
                for mkt, rows in mk.items():
                    ...
                    for r in rows:
                        ...
                        o = num(r.get("TDD_OPNPRC")); hi = num(r.get("TDD_HGPRC"))
                        lo = num(r.get("TDD_LWPRC")); cl = num(r.get("TDD_CLSPRC"))
                        if not code or not cap or cap <= 0 or rt is None:
                            continue
                        if not o or not hi or not lo or not cl or o <= 0 or lo <= 0:
                            continue
                        recs.append((cap, code, str(r.get("ISU_NM", "")).strip(), rt,
                                     int(o), int(hi), int(lo), int(cl)))
                    recs.sort(reverse=True)
                    for cap, code, name, rt, o, hi, lo, cl in recs[:TOPN[mkt]]:
                        day[code] = [o, hi, lo, cl, round(rt, 2)]
                ...
                empty_streak += 1
                if empty_streak >= 20:
                    log(f"[ABORT] 20 empty weekdays at {bas} - saving and stop")
                    break
